...
你對這世界感到好奇嗎？說一聲「學習」，選一座想探索的寶庫，我陪你慢慢走。

想知道我會做什麼，可以說「幫助」；若覺得光聽還不夠，就說一聲「挑戰」。寶庫的大門會為求知者開啟。"""

HELP_TEXT = """第一次來也不用拘謹。我是「永恆北極星」，可以陪你做六件事：

🌱 隨我走進寶庫
說「學習」選擇四條導引路線。先讀一小段、試著回答，再慢慢走向下一階段。五題挑戰答對四題便可進階，答錯也有解說；不必把分數當作對自己的評價。說「學習進度」查看、「暫停學習」休息，或「刪除學習進度」清除目前路線。

💬 日常聊天
聊心情、分享日常，或傳個顏文字都可以。我會接續最近三組對話；試煉中請先說「退出」再閒聊。
...
CHALLENGE_INTRO_TEXT = """原來如此，你想試試自己理解了多少。

老人將茶杯輕輕放下。沉睡的星圖逐一亮起，牆後傳來古老機關甦醒的低鳴。

「寶庫不拒絕求知之人。這五道題，是讓我們看清哪裡已經明白，哪裡值得再談。不是衡量你是否配得上知識。」
...
def render_final(*, score: int, total: int, best_streak: int) -> str:
    ...
    if ratio == 1.0:
        ...
        title = "🛰️ 深空領航者"
        judgment = "最後一道鎖扣落下。這次答得不錯；若能用自己的話解釋，再換個情境想想，知識便會更穩。"
    elif ratio >= 0.6:
        title = "🔭 星圖研習者"
