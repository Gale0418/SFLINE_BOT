...
- 題數是否恰為 300，ID 與題幹是否唯一，主題是否恰為 24 個。
- 原始基線是否仍為 96 題（16 主題 × 6 題），且沒有被擴充題覆寫。
- 原始 96 題是否維持四座正式寶庫各 24 題、每座 easy／medium／hard 各 8 題，以及每個寶庫／難度的 A、B、C、D 正解位置各兩題。
- 擴充的 204 題不強求平均分布，但每題仍須具備可追溯來源、唯一正解與合理難度。
- 「群星寶庫」是否能聚合四庫內容，並與四座正式寶庫共同形成 5 個入口；5 個入口 × 4 個難度的 20 條路徑都必須能完成。
...
- 對每個已支援的 AI provider／adapter，逾時、格式錯誤與配額不足都要有固定安全回覆；不能只測其中一家。
- Help 與 Quiz 不應依賴任何 AI provider 可用性。
...
- Mock、離線資料驗證、GitHub CI、手機 E2E 與各 provider 的線上模型評估必須分開標示。
- 沒有實測的項目不得寫成「通過」。
- 任何預填人工分數不得冒充已完成審核。
