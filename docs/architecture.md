...
**不做執行時跨供應商 fallback。** 這是刻意的成本防護：Google 免費額度或 API 發生錯誤時，不應自動開始消耗 OpenAI 付費額度。

Google Gemma 路徑直接呼叫 Gemini Developer API `generateContent`，使用 JSON structured output 與 `minimal` thinking；OpenAI 路徑使用 Responses API structured output。兩邊最後都經過相同的 `KnowledgeBase.validate_answer()` 語意驗證。
...
- 本專案本身仍不把使用者問題全文寫入日誌。
- 若未來部署為正式服務，應重新評估 Free Tier 資料使用條款或改用不使用內容訓練的付費方案。

OpenAI 與 Google 的服務條款、資料使用政策與價格可能改變；專題報告應記錄測試日期與實際 provider。
