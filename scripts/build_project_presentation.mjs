...
const workspace=path.resolve('.'),build=path.join(workspace,'.presentation-build','visual-v10'),output=path.join(workspace,'deliverables');
...
{
 const s=page('3. 系統架構與技術：整體流程',5);txt(s,'從手機訊息到 NAS 上可驗證的回覆',72,141,1136,46,31,C.red,true);const labels=[['LINE 使用者','文字／按鈕'],['Webhook','簽章驗證'],['事件排程','去重／依序'],['路由判斷','命令／問答'],['回覆 LINE','文字／Quick Reply']],xs=[62,294,526,758,990];for(let i=0;i<labels.length;i++){if(i<4)arrow(s,xs[i]+177,291,48,22,i===3?C.green:C.red);box(s,labels[i][0]+'\n'+labels[i][1],xs[i],244,184,118,{fill:i===0||i===4?'#E5ECEA':C.white,stroke:i===0||i===4?C.green:C.red,size:21})}rule(s,849,362,3,76);box(s,'規則路徑\nHelp、學習、計分',678,448,322,94,{fill:'#F0E2DF',stroke:C.red,size:22});box(s,'模型路徑\n開放問題與追問',1018,448,196,94,{fill:'#E7EBEF',stroke:C.blue,size:21});txt(s,'Synology NAS／Docker 常駐服務，ngrok 提供 LINE 可連線的 HTTPS Webhook。',72,574,1136,36,22,C.ink,true);txt(s,'LINE ID 轉成假名代碼；SQLite 與學習進度保存在 NAS 資料卷。',72,620,1136,30,18,C.muted);notes(s,4,[lineUrl,'本機 README.md、deploy/nas/、src/eternal_polaris/app.py、dispatcher.py、learning.py']);
}
{
