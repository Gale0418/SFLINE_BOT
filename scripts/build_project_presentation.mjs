...
const workspace=path.resolve('.'),build=path.join(workspace,'.presentation-build','visual-v4'),output=path.join(workspace,'deliverables');
...
const C={paper:'#F7F5F0',ink:'#242830',red:'#843C46',muted:'#66717A',white:'#FFFFFF',line:'#D8D1C7',gold:'#B9904A',blue:'#58748E',green:'#557869',violet:'#77658B',soft:'#EEE8DE'};
...
{
 const s=page('3. 系統架構與技術：整體流程',5);txt(s,'從手機訊息到可驗證回覆',72,141,1136,46,31,C.red,true);const labels=[['LINE 使用者','文字／按鈕'],['Webhook','簽章驗證'],['事件排程','去重／依序'],['路由判斷','命令／問答'],['回覆 LINE','文字／Quick Reply']],xs=[62,294,526,758,990];for(let i=0;i<labels.length;i++){if(i<4)arrow(s,xs[i]+177,291,48,22,i===3?C.green:C.red);box(s,labels[i][0]+'\n'+labels[i][1],xs[i],244,184,118,{fill:i===0||i===4?'#E5ECEA':C.white,stroke:i===0||i===4?C.green:C.red,size:21})}rule(s,849,362,3,76);box(s,'規則路徑\nHelp、學習、計分',678,448,322,94,{fill:'#F0E2DF',stroke:C.red,size:22});box(s,'模型路徑\n開放問題與追問',1018,448,196,94,{fill:'#E7EBEF',stroke:C.blue,size:21});txt(s,'明確指令交給程式，自然語言問題交給回答服務。',72,584,1000,43,24,C.ink,true);txt(s,'LINE ID 轉成假名代碼後使用；學習進度存在伺服器 SQLite',72,630,1040,30,18,C.muted);notes(s,4,[lineUrl,'本機 README.md、src/eternal_polaris/app.py、dispatcher.py、learning.py']);
}
{
 const s=page('3. 系統架構與技術：內容資料',6);txt(s,'1,236 張知識卡、300 題、24 個主題',72,139,1136,51,33,C.red,true);const chart=s.charts.add('bar',{position:{left:52,top:218,width:635,height:360},categories:['星海','地脈與生命','萬象法則','未來幻夢'],series:[{name:'題數',values:[161,37,30,72],fill:C.red}],barOptions:{direction:'bar',grouping:'clustered'},hasLegend:false,dataLabels:{showValue:true,position:'outEnd',textStyle:{fontSize:21,fill:C.ink}},xAxis:{textStyle:{fontSize:18,fill:C.ink}},yAxis:{textStyle:{fontSize:19,fill:C.ink}},chartFill:C.paper,plotAreaFill:C.paper});applyPresentationChartFont(chart,{fontFamily:font});shape(s,'ellipse',772,221,344,344,C.soft,C.line,1);shape(s,'ellipse',827,276,234,234,C.white,C.red,2);shape(s,'ellipse',884,333,120,120,C.ink,C.ink,0);txt(s,'四座寶庫',884,359,120,30,21,C.white,true,'center');txt(s,'80 段重組導引教材',810,511,270,28,19,C.red,true,'center');txt(s,'固定題庫不依賴 AI API',790,566,310,30,20,C.ink,true,'center');txt(s,'知識卡提供來源；題庫答案固定；導引教材安排學習順序。',72,615,1090,34,21,C.muted);notes(s,5,['本機 README.md、data/knowledge_cards.json、data/quiz_questions.tsv','docs/guided-learning.md']);
}
{
...
{
 const s=page('3. 系統架構與技術：導引式學習',8);txt(s,'「你對這世界感到好奇嗎？」',72,139,1136,51,31,C.red,true);box(s,'選擇路線',72,261,150,72,{fill:C.ink,stroke:C.ink,color:C.white,size:22});arrow(s,235,286,53,21);const routes=[['🌌 星海之庫',C.blue],['🌍 地脈與生命',C.green],['⚛️ 萬象法則',C.violet],['🚀 未來幻夢',C.gold]];routes.forEach((r,i)=>box(s,r[0],306,202+i*92,240,65,{stroke:r[1],size:21}));rule(s,563,235,3,298);const steps=[['短講','5 段'],['理解題','立即解說'],['過關挑戰','5 題答對 4 題'],['下一階段','各路獨立進度']];steps.forEach((r,i)=>{if(i<3)arrow(s,718+i*150,319,42,20);shape(s,'ellipse',615+i*150,270,112,112,i===3?'#E5ECEA':C.white,i===3?C.green:C.red,2);txt(s,String(i+1),649+i*150,286,44,30,20,C.red,true,'center');txt(s,r[0],627+i*150,320,88,28,18,C.ink,true,'center');txt(s,r[1],605+i*150,386,132,55,17,C.muted,false,'center')});txt(s,'四條路線各自保存進度，使用者可以自由換路再回來。',608,505,590,73,22,C.ink,true,'center');txt(s,'SQLite 位於機器人伺服器，LINE 手機端不執行 SQL。',608,592,590,30,18,C.muted,false,'center');notes(s,7,['本機 README.md、docs/guided-learning.md、src/eternal_polaris/learning.py、tests/test_learning.py']);
}
{
 const s=page('4. 系統驗證',9);txt(s,'LINE 機器人的主要流程有沒有照規則運作？',72,139,1136,51,31,C.red,true);const checks=[['1','入口安全','LINE 簽章\n偽造請求拒絕',C.red],['2','訊息一致','事件去重\nReply Token 只用一次',C.blue],['3','功能路由','幫助、試煉\n計分、退出、問答',C.violet],['4','進度保存','四條路線\nSQLite 交易',C.green]];rule(s,147,333,890,4);checks.forEach((c,i)=>{const x=105+i*286;shape(s,'ellipse',x,278,112,112,C.paper,c[3],5);txt(s,c[0],x+30,299,52,36,24,c[3],true,'center');txt(s,c[1],x-28,420,168,34,22,c[3],true,'center');txt(s,c[2],x-58,462,228,62,18,C.muted,false,'center')});shape(s,'roundRect',780,555,410,68,'#E5ECEA',C.green,2);txt(s,'2026-09-08：Gemma 26B 完成 4 次真實聊天測試',798,568,375,42,19,C.green,true,'center');txt(s,'4 次成功只代表 2026-09-08 當天的連線紀錄。',72,636,720,27,18,C.muted);notes(s,8,['本機 README.md、tests/','README 2026-09-08：gemma-4-26b-a4b-it 四次真實聊天 smoke test']);
}
{
 const s=page('5. 結論與未來工作',10);txt(s,'已完成成果、已知限制、尚未排定的下一步',72,139,1136,50,31,C.red,true);rule(s,125,349,1015,5);const nodes=[['問答','本機卡片＋固定試煉',C.red],['導引','四條路線＋進度',C.blue],['限制','模型需網路與額度',C.green],['下一步','目前尚未排定',C.gold]];nodes.forEach((n,i)=>{const x=105+i*292;shape(s,'ellipse',x,297,108,108,C.paper,n[2],5);txt(s,String(i+1),x+31,318,46,38,25,n[2],true,'center');txt(s,n[0],x-31,230,170,35,22,n[2],true,'center');txt(s,n[1],x-61,429,230,85,21,C.ink,true,'center')});txt(s,'已完成：1,236 張知識卡、300 題、80 段導引教材、伺服器端學習進度',72,564,1136,37,23,C.ink,true,'center');txt(s,'只陳述已完成的功能，不把可能方向寫成既定計畫。',72,615,1136,30,19,C.muted,false,'center');notes(s,9,[repo,'本機 README.md、docs/guided-learning.md']);
}
{
 ...
 const cards=[
  ...
  {p:'P5/P6/P8/P10',title:'專案 README、程式與資料',tag:'系統實作',content:'• 1,236 張知識卡與 300 題\n• 80 段導引教材\n• SQLite 與四路進度',stroke:C.violet,x:460,y:415},
...
const finalPath=path.join(output,'永恆北極星_全視覺導引學習版_v9.pptx');const result=await finalizePresentation({workspaceDir:workspace,candidatePath:candidate,finalPath,pythonExecutable:runtime+'/python/python.exe',integrityValidatorPath:skill+'/container_tools/inspect_presentation_package_integrity.py',layoutValidatorPath:skill+'/container_tools/inspect_presentation_layout_geometry.py',layoutArgs:['--expected-slide-size-emu','12192000,6858000','--validate-heading-fit'],explicitTotalSlideCount:11,requiredNativeTableOwnerSlides:[],requiredNativeChartOwnerSlides:[6],materializeLiteralChartWorkbooks:true,fontPolicy:{basis:'design',families:[font]},verifyArtifactToolImport:true,receiptPath:path.join(build,'validation-v9.json')});
const scriptPath=path.join(output,'永恆北極星_全視覺導引學習版_v9_逐頁演講稿.md');const intro='# 永恆北極星：逐頁演講稿（全視覺導引學習版）\n\n對應檔案：永恆北極星_全視覺導引學習版_v9.pptx，共 11 頁。預計約 30 分鐘，包含操作與互動；請依實際語速排練。\n\n## 講者設定\n\n講者採聰慧、俐落、略帶活潑的天才少女學生風格。機器人本身維持沉穩而慈祥的睿智老人。\n\n';await fs.writeFile(scriptPath,intro+speech.map((n,i)=>`## 第 ${i+1} 頁　${n.title}\n\n${n.body}\n`).join('\n'),'utf8');console.log(JSON.stringify({final:result.finalPath,slides:p.slides.items.length,script:scriptPath}));
