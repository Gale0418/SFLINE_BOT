...
- **Google AI Studio / Gemini Developer API**：預設推薦 `gemma-4-31b-it`。Gemma 4 31B IT 可透過 Gemini API 代管使用；目前 Gemma 4 Free Tier 的輸入與輸出皆免費。
...
請到 Google AI Studio 建立新的 Gemini API key。2026 年 9 月起 Google 正在淘汰舊式 unrestricted standard keys；新建立的 key 會使用新的 Auth key 流程。
...
兩條路徑都使用相同的 24 張人工知識卡、相同 JSON 輸出契約與相同程式端驗證；Google 端使用 Gemma 4 的 `minimal` thinking，以降低延遲。
...
- Google Gemma 4 31B `generateContent`、Gemini API key header、JSON Schema 與 `minimal` thinking。
